...
function generateRecommendations(w: WeatherData): Rec[] {
  ...
  if (['Rain', 'Drizzle'].includes(condition))
    recs.push({ type: 'warning', icon: '☂️', title: 'Rain Expected', text: 'Bring an umbrella or waterproof jacket. Roads may be slippery.' });
  if (condition === 'Thunderstorm')
    recs.push({ type: 'warning', icon: '⚡', title: 'Thunderstorm Alert', text: 'Stay indoors. Avoid open fields, tall trees and water.' });
  if (condition === 'Snow')
  ...
  if (condition === 'Fog' || visibility < 1000)
    recs.push({ type: 'warning', icon: '🌫️', title: 'Low Visibility', text: 'Use fog lights when driving and reduce speed.' });
  if (windSpeed > 15)
    recs.push({ type: 'warning', icon: '💨', title: 'Strong Winds', text: 'Secure loose outdoor objects. Avoid cycling in exposed areas.' });

  if (condition === 'Clear' && temperature > 20)
    recs.push({ type: 'health', icon: '🕶️', title: 'UV Protection', text: 'Apply SPF 30+ sunscreen and wear sunglasses.' });
  if (humidity > 85 && temperature > 25)
    recs.push({ type: 'health', icon: '💧', title: 'High Humidity', text: 'Stay hydrated. Limit strenuous outdoor activity during peak heat hours.' });
