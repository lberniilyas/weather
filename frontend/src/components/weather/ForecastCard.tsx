...
interface CardProps { day: ForecastDay }
...
function ForecastCard({ day }: CardProps) {
  ...
  return (
    <div className="glass rounded-2xl p-5 flex flex-col items-center gap-3 hover:bg-white/10 transition-colors group">
      <p className="text-slate-400 text-xs font-medium">{label}</p>
      <img src={icon} alt={day.condition} className="w-14 h-14 drop-shadow group-hover:scale-110 transition-transform" loading="lazy" />
